import sys
import os
path_to_lib = os.path.abspath(os.path.join(os.path.dirname(__file__), '..')) + '\\lib'
...
class WhileParser(object):
    ...
    TOKENS = r"(if|then|else|while|do|skip|assert)(?![\w\d_])   (?P<id>[^\W\d]\w*)   (?P<num>[+\-]?\d+)   (?P<op>[!<>]=|([+\-*/<>=]))    [();]  := (?P<hole>(\?\?))".split()
    GRAMMAR = r"""
    S   ->   S1     |   S1 ; S
    ...
    def postprocess(self, t):
        ...
        elif len(t.subtrees) == 3 and t.subtrees[0].root == '(':  # remove brackets
        ...
        elif t.root == 'S1' and t.subtrees[0].root == 'assert':  # handle assert statement
...
if __name__ == '__main__':
    ast = WhileParser()('y := 10 ; while y > 0 do  y := y - 1; y := y + 10')
    if ast:
        print(">> Valid program.")
        print(ast)
    else:
